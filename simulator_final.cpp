...
{
private:
    int queue[n] = {0};            // running queue for the simulator .stores the process ids.
    int ready_queue[n] = {0};      // to store the processes id which are ready to get executed at the given point of time
    int ready_queue_top = -1;      // index to indicate the top element of ready_queue .just like top in queue.
    int queue_front_index;         // index to the top element in queue
    int init_time = -1;            // used to store the time the runnng process started its execution
    int min_burst_time = MAX_SIZE; // to store the minimum burst time required in STRF nad round robin
